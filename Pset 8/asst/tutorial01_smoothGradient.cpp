...
int main(int argc, char** argv)
{
    ...
    // (The string in parenthesis is the name that
    // will be used for this variable when Halide prints debugging information)
    Var x("x");
    Var y("y");
    ...
    for (int j=0; j<output.height(); j++) {
        for (int i=0; i<output.width(); i++) {
            ...
            if (output(i, j) != i+j) {
                cout << "Something went wrong at (" << i << "," << j << ") " << output(i, j) << endl;
                return EXIT_FAILURE;
            }
        }
    }
