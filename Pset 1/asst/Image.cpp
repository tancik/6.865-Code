...
long long Image::number_of_elements()const {
    // --------- HANDOUT  PS01 ------------------------------
    // returns the number of elements in the image. An RGB (3 color channels)
    // image of 100 × 100 pixels has 30000 elements'
    return width() * height() * channels(); // change this
}
...
const float & Image::operator()(int x, int y) const {
    ...
    return Image::operator()(x + y * width());
}
...
const float & Image::operator()(int x, int y, int z) const {
    ...
    return Image::operator()(x + y * width() + z * width() * height());
}
